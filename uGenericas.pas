...
uses
  ...
  Db, StdCtrls, DateUtils, DBGrids, Buttons, Grids, DBClient, DBCtrls,
  ExtCtrls, WinINet, TLHelp32, Math, IniFiles, Printers, ShellApi, ComCtrls, Registry;

  Function  InfoSistemaOperacional(var pCPU:String; var pUser:String; var pStat:Integer):Boolean;
  ...
  Function  ValidaConfiguracaoMinimaTela(pmtLargura,pmtAltura:Integer;pmtCancel:Boolean=True): Boolean;

  Function  AjustaPathBarraFinal(pmtPath:String;pSN:Boolean): String;
  Procedure AtivaBlocodeNotas(pmtArqTxt:String);
  Procedure FonteOrientacaoBlocodeNotas(pFontName:string;pFontSize:Integer);

  Procedure EncadeamentoExecutavel(pmtExec,pmtParm:String; pmtForma:String='Shell'; pmtShow:Integer=1);
  ...
  Procedure Form_DefineOrig(pmtForm:TForm; pTop,pLeft,pHeight,pWidth:Integer; pCentrar:Boolean=False);
  Procedure Form_SimplesDefinicao(pmtForm:TForm; pHeight,pWidth:Integer;pBarra:Integer=0);
  ...
  Procedure Form_CentralizaTela(pmtForm:TForm;pmtButton:TMouseButton;pHeight,pWidth:Integer;pTaman:Integer=4);
  Procedure Form_Deleta(pmtForm:TForm);
  ...
  Procedure AppendDAT(pArqDAT:String; pInfo:String);
  ...
  Function  TrocaCaracter(pTexto:String;pOld,pNew:String):String;
  ...
  Function  ValidaHorario(pHora:String; pVazio:Boolean=True):Boolean;
  ...
  Function  EditaNCM(pDado:String; nTam:Integer=11): String;
  ...
  Function  ValidaDVCodBar(pmtCodBar:String;pmtMsg:Boolean=False): Boolean;
  Function  ValidaFCI(pFCI:String;pMsg:Boolean):Boolean;
  Function  ValidaEMail(pEMailIn:String; pObrigatorio:Boolean=False):Boolean;
  ...
  Function  DataSetState(pState:TDataSetState): String;

  Function  DefineGrid(pGrid:TDBGrid; pLarg:Array of Real; pCol1,pCol2:Integer; pPerc:Boolean = True): TDBGrid;
  Procedure DestaqueGrid(pGrid:TDbGrid; pCor,pCorDest,pTit,pTitDest:TColor; pCol:Array of Integer);
  ...
  Function  ConjuntoEspecial(pDado:String;pValid:String;pUpperCase:Boolean=True;pTroca:String=' '): String;

  Function  TrocaCaracteres(var pDado:String; pCarac:String; pTroca:String): Boolean;
  Function  ExclueCaracteres(pDado:String; pCarac:String): String;
  ...
  Function  EditaChaveNFSe(pChaveNFSe:String; pMunic:Integer=5108):String;
  ...
  Function CalculaJuros(pVlrEmissao,pPcJuros:Real;
                      pTipoJuros:Integer;
                      pDtVencto,pDtCalculo:TDateTime): Real;
  ...
  Procedure Identificacao(pmtTexto:String; var pmtIdentif:String; pmtSenha:Boolean=False);
  Function  ValidaSenha(pmtMotivo,pmtSenha:String;pmtDifMinMai:Boolean=False): Boolean;

  Procedure HintsDBNavigator(pmtNavigator:TDBNavigator; pmtDescrCompleta:Boolean = True);
  Function RE_ObterLinhaAtual(RichEdit: TRichEdit): Integer;
...
type
  ...
  public
    { Public declarations }
    lSalvaForm,lDebug,lDebugNFe: Boolean;
...
var
  ...
  AA,MM,DD: Word;
  wTpInfo: Integer;    // 1-Identificacao....   2-ValidaSenha
  wSenhaInfo: String;
  wDifMinMai,wSenhaResult: Boolean;
...
// Obtem dados do sistema operacional - User logado
Function  InfoSistemaOperacional(var pCPU:String; var pUser:String; var pStat:Integer):Boolean;
...
begin
  ...
  Try
    GetMem(cNome,nSize);
    if GetComputerName(cNome,nSize) then pCPU := cNome;
  Finally
    FreeMem(cNome,nSize);
  End;
  // Usuário logado
  nSize  := 120;
  if GetUserName(lpBuffer,nSize) then pUser := lpBuffer
                                 else pStat := GetLastError();
  //
  Result := False;
  if (pCPU <> 'NI') and (pUser <> 'NI') and (pStat = 0) then Result := True;
...
begin
  Result := True;
  wMsg := '';
  if (Screen.Width < pmtLargura) or (Screen.Height < pmtAltura)
  then begin
    wMsg := 'Configuração de exibição não atende requisitos mínimos' + #13 +
            '   Requisito mínimo   Largura=' + IntToStr(pmtLargura) + '   Altura=' + IntToStr(pmtAltura) + #13 +
            '   Detectado   Largura=' + IntToStr(Screen.Width) + '   Altura=' + IntToStr(Screen.Height) + #13#13;
    if pmtCancel
    then wMsg := wMsg + 'Aplicação cancelada' + #13;
...
// IncludeTrailingPathDelimiter  ou  ExcludeTrailingPathDelimiter
var iMax: Integer;
begin
  Result := Trim(pmtPath);
  iMax := Length(Result);
...
begin
  ...
  then MessageDlg('Arquivo [' + pmtArqtxt + '] não encontrado',mtError,[mbOk],0)
    ...
    if WinExec(PAnsiChar(Comando), SW_SHOWDEFAULT) < 31 then
      MessageDlg('NotePad não encontrado',mtWarning, [mbOK], 0);
  end;
...
Procedure FonteOrientacaoBlocodeNotas(pFontName:string;pFontSize:Integer);
var wReg: Tregistry;         // "registry" no uses...
    regFont: String;
    regSize,regOrientacao: Integer;
begin
  Try
    ...
    do begin
      ...
      if pFontName <> ''
      then begin
        regFont := readstring('lfFaceName');
        if regFont <> pFontName
           then WriteString('lfFaceName', pFontName);
      end;
      //
      pFontSize := pFontSize * 10;
      regSize := ReadInteger('iPointSize');
      if regSize <> pFontSize
         then WriteInteger('iPointSize', pFontSize);
      //
      Closekey;
    end;
  Except
...
Procedure EncadeamentoExecutavel(pmtExec,pmtParm:String; pmtForma:String='Shell'; pmtShow:Integer=1);
// pmtForma=SHELL; WINEXEC
// pmtShow=0:Hide 1:Normal 3:Maximize
var wExt,wCmd,wPrm,wDir,wForma: String;
    wMsgErro: String;
begin
  wCmd := pmtExec;
  ...
  then wMsgErro := pmtExec + ' não é um executável' + #13
  else if not FileExists(wCmd)
       then wMsgErro := wCmd + ' não encontrado' + #13;
  //
  wForma := AnsiUpperCase(pmtForma);
  if (wForma <> 'SHELL') and (wForma <> 'WINEXEC')
     then wMsgErro := wMsgErro + 'Forma: ' + pmtForma + ' (SHELL ou WINEXEC)' + #13;
  if (pmtShow <> 0) and (pmtShow <> 1) and (pmtShow <> 3)
     then wMsgErro := wMsgErro + 'Show: ' + IntToStr(pmtShow) + ' (0, 1 ou 3)' + #13;
  if wMsgErro <> ''
  then begin
    MessageDlg('EncadeamentoExecutavel' + #13 +
               'pmtExec=' + pmtExec + '  - Não ativado' + #13 +
               'pmtParm=' + pmtParm, mtError, [mbOk], 0);
    Exit;
  end;
  if wForma = 'SHELL'
  then begin
...
begin
  ...
  if FileExists(wArqForm) then
     DeleteFile(wArqForm);
  wForm        := TIniFile.Create(wArqForm);
  wSection     := pmtForm.Name;
  wInfo        := 'alNone';
  if (pmtForm.Align = alClient) or ((pmtForm.Top < 0) and (pmtForm.Left < 0))
     then wInfo := 'alClient';
  //
  ...
  nBorda := 2;    // bsSizeable
  for i := 0 to 5 do
    if pmtForm.BorderStyle = TFormBorderStyle(i)
    then begin
      nBorda := i;
      Break;
    end;
  wForm.WriteInteger(wSection,'BorderStyle', nBorda);
  wForm.Free;
...
begin
  ...
  then begin    // Define janela na dimensão máxima
    pmtForm.BorderStyle := TFormBorderStyle(2);     // bsSizeable
  ...
  if (not FGen.lSalvaForm)
     ...
     then Exit;
  wArqForm := AjustaPathBarraFinal(FGen.pathSalvaForm,True) + '_' + pmtForm.Name + '.fml';
  if not FileExists(wArqForm) then
     Exit;
  wForm        := TIniFile.Create(wArqForm);
  wSection     := pmtForm.Name;
  wInfo        := wForm.ReadString(wSection, 'Align',  'alNone');
  nBorda       := wForm.ReadInteger(wSection,'BorderStyle', 2);
  ...
  pmtForm.BorderStyle := TFormBorderStyle(nBorda);
  wForm.Free;
...
begin
  ...
  if pHeight > nAltMax then
    pHeight := nAltMax;
  if pWidth > nLargMax then
    pWidth  := nLargMax;
  if pCentrar then
  begin
  ...
  if xPos = 'T' then          // Barra de tarefas no topo
    pTop := pTop + barAlt
    else if xPos = 'L' then       // Barra na esquerda
           pLeft := pLeft + barLrg;
  pmtForm.Align    := alNone;
  pmtForm.Top      := pTop;
  pmtForm.Left     := pLeft;
  ...
  pmtForm.Position := poDesigned;
  if FGen.lSalvaForm and DirectoryExists(FGen.pathSalvaForm) then
    Form_Salva(pmtForm);
...
begin
  pmtForm.Align  := alNone;
  wTop   := (Screen.Height - (pHeight + pBarra)) div 2;
...
Procedure Form_CentralizaTela(pmtForm:TForm;pmtButton:TMouseButton;pHeight,pWidth:Integer;pTaman:Integer=4);
//  pmtForm:    Nome do formulário
//  pmtButton:  Botão pressionado (mbleft ou mbRight)
//  pHeight:    Altura do formulário
//  pWidth:     Largura do formulário
//  pTaman:     Dimensão da ser adotada 0-Desconsidera  1-Tela cheia  2-Meia-tela  3-Dois terços  4-Tres quartos  5-Quatro quintos....
//              Válido SOMENTE para pmtButton = mbLeft  (botão esquerdo)
var iMult: Integer;
begin
  pmtForm.Align := alNone;
  if (pmtButton = mbLeft) and (pTaman <> 0)
  then begin
    iMult := pTaman - 1;
    if iMult < 0 then iMult := 1;
    if pmtForm.Height >= Screen.Height then pmtForm.Height := Screen.Height - 32;
    if pmtForm.Width >= Screen.Width then pmtForm.Width := Screen.Width - 64;
    pmtForm.Height := (Screen.Height div pTaman) * iMult;
    pmtForm.Width := (Screen.Width div pTaman) * iMult;
  end
  else begin
    if pHeight >= Screen.Height then pHeight := Screen.Height - 32;
    if pWidth >= Screen.Width then pWidth := Screen.Width - 64;
    pmtForm.Height := pHeight;
    pmtForm.Width := pWidth;
  end;
  pmtForm.Top := (Screen.Height - pmtForm.Height) div 2;
  pmtForm.Left := (Screen.Width - pmtForm.Width) div 2;
...
begin
  if pLogFile = '' then
  begin
    MessageDlg('Arquivo log não informado',mtWarning,[mbOk],0);
    Exit;
  end;
...
Procedure AppendDAT(pArqDAT:String; pInfo:String);
{
  pArqDAT --> Nome completo do arquivo de texto
  pInfo   --> Texto a ser adicionado
}
var txtDAT: TextFile;
    naL: Integer;
begin
  if pArqDAT = '' then
  begin
    MessageDlg('Arquivo DAT não informado',mtWarning,[mbOk],0);
    Exit;
  end;
  if FileExists(pArqDAT)
  then naL := 0
  else begin
    naL := FileCreate(pArqDAT);
    FileClose(naL);
  end;
  if naL < 0
  then begin
    MessageDlg('Não foi possível criar arquivo DAT' + #13
               + '[' + pArqDAT + ']'
               ,mtWarning,[mbOk],0);
    Exit;
  end;
  AssignFile(txtDAT,pArqDAT);
  Append(txtDAT);
  WriteLn(txtDAT,pInfo);
  Close(txtDAT);
...
begin
  Result  := 0;
  OrigSep := FormatSettings.DecimalSeparator;                     // FormatSettings.DecimalSeparator (Delphi 10)
  if OrigSep = ','
     then dSepar := '.'
     else dSepar := ',';
  Try
  ...
  Except
    FormatSettings.DecimalSeparator := dSepar;                   // FormatSettings.DecimalSeparator (Delphi 10)
    Try
      Result := StrToFloat(pArgum);
    Except
    End;
    FormatSettings.DecimalSeparator := OrigSep;                 // FormatSettings.DecimalSeparator (Delphi 10)
  End;
...
Function TrocaCaracter(pTexto:String;pOld,pNew:String):String;
// pTexto: String a ser tratado
// pOld: Caracter a ser substituido
// pNew: Caracter substituto
// Retorna o texto tratado
var i: Integer;
    xCar: String;
begin
  Result := pTexto;
  if Pos(pOld,pTexto) = 0 then Exit;
  //
  Result := '';
  for i := 1 to Length(pTexto)
  do begin
    xCar := Copy(ptexto,i,1);
    if xCar = pOld
       then xCar := pNew;
    Result := Result + xCar;
  end;
...
begin
  ...
  if pmtExt then Result := Result + FormatSettings.LongMonthNames[wMes]
            else Result := Result + FormatSettings.ShortMonthNames[wMes];
...
begin
  ...
  case pForma of
    ...
    4:Result := xAno + xMes + xDia + '_' + xHor + xMin + xSeg;                                     // 13 ou 15 AA(AA)MMDD_HrMiSe(Default)
    5:Result := xAno + xMes + xDia + XHor+ xMin;                                                      // 12 ou 10 AA(AA)MMDDHrMi
    6:Result := xAno + xMes + xDia;                                                                   //  8 ou  6 AA(AA)MMDD
    else Result := xAno + xMes + xDia + XHor+ xMin + xSeg;                                               // 14 ou 12 AA(AA)MMDDHrMiSe
...
begin
  Result := True;
  if (Length(Trim(pTpess)) = 0) and (Length(Trim(pInfo)) = 0)
    then Exit;
  //
  xCNPJCPF := Numerico(pInfo);
  if Length(Trim(pTpess)) = 0 then
    if Length(Trim(xCNPJCPF)) > 11 then
      pTpess := 'J'
    else
      pTpess := 'F';
  //
  if AnsiUpperCase(pTpess) = 'J' then
    Result := ValidaCNPJ(xCNPJCPF)
  else if AnsiUpperCase(pTpess) = 'F' then
         Result := ValidaCPF(xCNPJCPF)
       else
         Result := False;
  //
  if Result and (not lSeq) then
  begin
    nP := 0;
    for i := 0 to 9 do
    begin
...
Function  EditaNCM(pDado:String; nTam:Integer=11): String;
begin
  while Length(pDado) < 8 do pDado := pDado + '0';
  if nTam = 11 then Result := Copy(pDado,1,2) + '.' + Copy(pDado,3,2)
               else Result := Copy(pDado,1,4);
  Result := Result + '.' + Copy(pDado,5,2) + '.' + Copy(pDado,7,2);
...
//         GTIN-8, GTIN-12, GTIN-13, GTIN-14, GSIN, SSCC
// pmtMsg: Exibe mensagemcom o dv informado (últimoO e calculado
...
begin
  ...
  if (nTam < 8)
     ...
     or (nTam > 18)
  then begin
    if pmtMsg then MessageDlg('Tamanho do código de barras inválido: ' + IntToStr(nTam) + #13 +
                              'Pode ser 8, 12, 13, 14, 17 ou 18',mtError,[mbOk],0);
    Exit;
  end;
  wSoma  := 0;
  ...
  if wSoma > 0
  then begin
    if pmtMsg then MessageDlg('Código de barras contém caracter não-numérico',mtError,[mbOk],0);
    Exit;
  end;
  //
  ...
  then Result := True
       ...
       then MessageDlg('DV Código de barras inválido' + #13 + 'Informado: ' + pmtCodBar[18] + '   Calculado: ' + IntToStr(wDig),
                       mtError,[mbOk],0);
...
begin
  if (not pObrigatorio) and (pEMailIn = '') then
  begin
    Result := True;
    Exit;
  end;
...
begin
  ...
  if StrToIntDef(pNroUso,0) = 0 then Exit;
  //
  Result  := 'F';    // Fixo
  if Length(Trim(pNroUso)) < 8 then Exit;
  if Length(Trim(pNroUso)) = 8
  then begin
    if (pNroUso[1] = '9') or (pNroUso[1] = '8') or (pNroUso[1] = '7')
    then begin
      pNroUso := '9' + pNroUso;
      Result  := 'C';      // Celular
    end;
...
begin
  ...
  for i := 1 to Length(pCol)
  do begin
    nC := pCol[i-1];
    pGrid.Columns[nC].Color       := pCorDest;
    pGrid.Columns[nC].Title.Color := pTitDest;
  end;
...
begin
  infDd := Trim(pDado);
  if pUpper then
     infDd := AnsiUpperCase(infDd);
  Result := '';
  for i := 1 to Length(infDd)
  do begin
    if ((infDd[i] >= '0') and (infDd[i] <= '9'))
       or ((infDd[i] >= 'A') and (infDd[i] <= 'Z'))
       or ((infDd[i] >= 'a') and (infDd[i] <= 'z')) then
      Result := Result + infDd[i];
  end;
...
const wValidar1: String = ('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz.,@#$%&*()-+=:;\/_ ');
      ...
      wValidar3: String = ('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz.,@#$%&*()-+_');
      wValidar4: String = ('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_');
begin
  Result := '';
  infDd  := Trim(pDado);
  if pUpper then
  ...
  case pValid of
    ...
    2:xValid := wValidar2;
    3:xValid := wValidar3;
    else xValid := wValidar4;
  end;
  for i := 1 to Length(infDd)
  do if Pos(infDd[i],xValid) > 0 then
       Result := Result + infDd[i];
...
Function EditaChaveNFe(pArgum:String; pEditaVazio:Boolean = False): String;
begin
  pArgum := Numerico(pArgum);
  if (Length(Trim(pArgum)) = 0) and (not pEditaVazio)
  then begin
    Result := '';
    Exit;
  end;
  while Length(pArgum) < 44
  do pArgum := pArgum + '0';
  Result := Copy(pArgum,1,4) + '.'
            + Copy(pArgum,5,4) + '.'
            + Copy(pArgum,9,4) + '.'
            + Copy(pArgum,13,4) + '.'
            + Copy(pArgum,17,4) + '.'
...
begin
  case pMunic of
    5108:begin
           while Length(Trim(pChaveNFse)) < 39
           do pChaveNFSe := pChaveNFSe + '0';
           Result := Copy(pChaveNFSe,1,2) + '-' +
                     Copy(pChaveNFSe,3,14) + '-' +
                     Copy(pChaveNFSe,17,2) + '-' +
                     Copy(pChaveNFSe,19,3) + '-' +
                     Copy(pChaveNFSe,22,9) + '/' +
                     Copy(pChaveNFSe,31,9);
         end;
    else Result := pChaveNFSe;
  end;
...
Function CalculaValorAtual(pVlrEmissao,pVlrDesconto,pPcJuros,pVlrMulta,pVlrPago:Real;
                           ...
                           pConcDesc:Boolean=False): Real;
{
  Cálculo do valor atualizado de um documento
    Parametros:
    ...
    Uso: VlrAtual :=  CalculaValorAtual(pVlrEmissao, pVlrDesconto, pPcJuros, pVlrPago,
                                        pTipoJuros,
                                        pDtVencto, pDtDesconto, pDtCalculo, False)
}

var nDias: Integer;
    wDesconto,wJuros,wJurDia: Real;
    wVlrFinal: Real;
begin
  ...
  wVlrFinal := pVlrEmissao - wDesconto;                 // Vlr. nominal no momento

  // Verifica se há juros (Tipo > 0) e se há atraso (nDias > 0)
  wJuros  := 0;
  if (pTipoJuros > 0) and (nDias > 0)
  then begin
    wJuros := CalculaPercentual(wVlrFinal, pPcJuros, 4);       // Juro mensal ou diario
    // pTipoJuros  1=Diario   2=Mensal     [wJuros := (Trunc((wJuros / 30) * 100) / 100) * nDias]
    if pTipoJuros = 2 then
    begin
      wJurDia := wJuros / 30;
      wJurDia := Trunc(wJurDia * 10000) / 10000;
    end
    else wJurDia := wJuros;
    wJuros := Trunc(wJurDia * nDias * 100) / 100;
  end;
  wVlrFinal := wVlrFinal + wJuros;

  // Se houve atraso, considerar o valor da multa
  if nDias > 0 then
    wVlrFinal := wVlrFinal + pVlrMulta;

  // Se há concessão de desconto
  if pConcDesc then
    wVlrFinal := wVlrFinal - pVlrDesconto;

  // Valor final, considera valor anteriormente pago
  wVlrFinal := wVlrFinal - pVlrPago;
  if wVlrFinal < 0 then
    wVlrFinal := 0;

  Result := Trunc(wVlrFinal * 100) / 100;
...
Function CalculaJuros(pVlrEmissao,pPcJuros:Real;
                      ...
                      pDtVencto,pDtCalculo:TDateTime): Real;
{
  Calcula o juro do titulo
    Parametros:
       pVlrEmissao  -> Valor nominal do titulo, emissão
       pPcJuros     -> % de juros (cfe. Diario ou Mensal),
       pTipoJuros   -> Tipo de juros (0-Sem juros, 1-Diario, 2-Mensal),
       pDtVencto    -> Data de vencimento considerado, verificar ANTES se é FERIADO   + Dias de isenção
       pDtCalculo   -> Data atual (para cálculo)
    Resultado:
       Valor dos juros a considerar
    Uso: VlrJuros :=  CalculaJuros(pVlrEmissao, pPcJuros,
                                   pTipoJuros,
                                   pDtVencto, pDtCalculo);
}
var nDias,iJuros: Integer;
    wJuros,wJurDia: Real;
begin
  Result := 0;
  if pTipoJuros = 0                     // Não há juros na parcela/titulo
     then Exit;
  while not DiaUtil(pDtVencto)
    do pDtVencto := pDtVencto + 1;      // Se data NÃO é dia útil, adianta um dia na data de vencimento
  pDtVencto := DateOf(pDtVencto);
  pDtCalculo := DateOf(pDtCalculo);
  if pDtVencto >= pDtCalculo then
    Exit;
  //
  nDias := DaysBetween(pDtCalculo, pDtVencto);
  if nDias <= 0
     then Exit;                         // Antes ou até o vencimento
  //
  wJuros := CalculaPercentual(pVlrEmissao, pPcJuros, 4);     // Valor do juro (diario ou mensal)
  if pTipoJuros = 2
     then wJurDia := wJuros / 30
     else wJurDia := wJuros;
  wJuros := Trunc(wJurDia * nDias * 100) / 100;
...
begin
  ...
  begin
    ...
    if pmtSenha then
    begin
      edInfo.CharCase := ecUpperCase;
      edInfo.PasswordChar := '#';
    end
    else begin
      edInfo.CharCase := ecNormal;
      edInfo.PasswordChar := #0;
    end;
    ShowModal;
...
begin
  ...
  wTpInfo := 2;
  wSenhaInfo := pmtSenha;
  wDifMinMai := pmtDifMinMai;
  if not pmtDifMinMai then
    wSenhaInfo := AnsiUpperCase(pmtSenha);     // Não diferencia entre Minusculo e Maiusculo
  with FGen do
  begin
    Caption := 'Informe:';
    LabOperacao.Caption := pmtMotivo;
    edInfo.Text := '';
    edInfo.PasswordChar := '#';
    imgSenha.Visible := True;
    FGen.ShowModal;
    edInfo.PasswordChar := ' ';
    imgSenha.Visible := False;
    Result := wSenhaResult;
  end;
...
Procedure HintsDBNavigator(pmtNavigator:TDBNavigator; pmtDescrCompleta:Boolean = True);
var i: Integer;
const txtComp: array [0..9] of String = ('Primeiro registro',  'Registro anterior', 'Próximo registro', 'Último registro',
                                         'Incluir registro',   'Excluir registro',  'Editar registro',  'Gravar registro',
                                         'Cancelar edição', 'Atualizar dados');
      txtAbrv: array [0..9] of String = ('Primeiro', 'Anterior', 'Próximo', 'Último',   'Incluir',
                                         'Excluir',  'Editar',   'Gravar',  'Cancelar', 'Atualizar');

begin
  pmtNavigator.Hints.Clear;
  for i := 0 to 9
  do if pmtDescrCompleta then pmtNavigator.Hints.Add(txtComp[i])
                         else pmtNavigator.Hints.Add(txtAbrv[i]);
  pmtNavigator.ShowHint := True;
...
begin
  ...
  begin
    ...
    begin
      if (RichEdit.CaretPos.Y = i+1) then
      begin
        Result := i+1;
        Break;
      end;
...
begin
  ...
  if wTpInfo = 2
  then begin
    if not wDifMinMai then
      wAuxil := AnsiUpperCase(edInfo.Text)
    else
      wAuxil := edInfo.Text;
    if wAuxil <> wSenhaInfo then
    begin
      LabErro.Visible := True;
      LabErro.Caption := 'Senha inválida, reinforme ou cancele';
      edInfo.SetFocus;
      Exit;
    end
    else
...
procedure TFGen.FormActivate(Sender: TObject);
begin
  edInfo.SetFocus;

end;

procedure TFGen.ImgSenhaClick(Sender: TObject);
begin
  if edInfo.PasswordChar = '#' then
    edInfo.PasswordChar := chr(0)
  else
    edInfo.PasswordChar := '#';
